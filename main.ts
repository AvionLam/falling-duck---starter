...
game.onUpdate(function () {
    if (duck.bottom > 135 || duck.top < -10 ) {
        game.over(false)
    }
})
...
game.onUpdateInterval(1500, function() {
    ...
    let topProjectile = sprites.createProjectileFromSide(topImage, -45, 0)
    let bottomProjectile = sprites.createProjectileFromSide(bottomImage, -45, 0)
    topProjectile.top = 0 
    bottomProjectile.bottom = scene.screenHeight()

})

sprites.onOverlap(SpriteKind.Player, SpriteKind.Gap, function (duck, gap) {
    if (gap.right - duck.left < 2) {
